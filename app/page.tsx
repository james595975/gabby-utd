...
interface MatchData {
  home_team: string;
  away_team: string;
  home_score: number; // ⚠️ 스크린샷 기준으로 H가 대문자일 수 있으니 대응
  away_score: number;
  date?: string;
}

export default function HomePage() {
  const matchId = "1"; // Supabase에 넣은 데이터 ID
  ...
  useEffect(() => {
    async function fetchMatchData() {
      ...
      // ⚠️ 만약 Supabase 컬럼명이 전부 소문자라면 아래 Home_score를 home_score로 바꾸세요!
  ...
  // 데이터가 없을 때 보여줄 기본값 (Supabase 컬럼 대소문자 매핑 포함)
  const displayHomeTeam = match?.home_team || '계비 UTD';
  const displayAwayTeam = match?.away_team || '잔뇨 FC';
  const displayHomeScore = match?.home_score ?? match?.['home_score'] ?? 7; 
