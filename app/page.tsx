...
interface MatchData {
  ...
  home_score: number;
  away_score: number;
  home_logo?: string; // ⚽ 사진 URL 컬럼 추가
  away_logo?: string; // ⚽ 사진 URL 컬럼 추가
  date?: string;
}
  ...
  return (
    <div className="bg-[#4a1525] text-white min-h-screen font-sans antialiased">
      ...
      <section className="flex flex-col items-center justify-center text-center pt-20 pb-16 px-4">
        {/* 구단 대표 메인 로고 영역 (만약 여기도 사진을 넣고 싶다면 match?.home_logo 사용 가능) */}
            ...
            <div className="flex items-center justify-between px-6 py-6">
              ...
              <div className="flex flex-col items-center w-5/12 text-center">
                <div className="w-14 h-14 rounded-full bg-black/20 border border-white/10 flex items-center justify-center overflow-hidden mb-2 shadow-inner">
                  {match?.home_logo ? (
                    <img src={match.home_logo} alt="Home Team Logo" className="w-full h-full object-cover" />
                  ) : (
              ...
              <div className="flex flex-col items-center w-5/12 text-center">
                <div className="w-14 h-14 rounded-full bg-black/20 border border-white/10 flex items-center justify-center overflow-hidden mb-2 shadow-inner">
                  {match?.away_logo ? (
                    <img src={match.away_logo} alt="Away Team Logo" className="w-full h-full object-cover" />
                  ) : (
                    <span className="text-2xl">🏃</span>
                  )}
                </div>
      ...
      {/* 3. 선수 명단 및 4. 연락하기 섹션 생략 (위 기존코드와 동일하게 유지됨) */}
      <section className="max-w-md mx-auto px-4 mb-16">
